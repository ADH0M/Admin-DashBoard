...
const Aside = () => {
  ...
  return (
    <motion.div
        className = {`flex-shrink-0 transition-all duration-300 ease-in-out  h-full ${isSidebarOpen?'w-64':'w-20'} z-10`} 
        animate   = { {width:isSidebarOpen?'256px':'80px'} }
    >
        <div className="flex flex-col h-full border-r p-2 border-[#cdc9f8b6]  bg-[#e0e4ff] bg-opacity-10 backdrop-blur-sm">
            <motion.div
            ...
            <nav className="flex-grow mb-8">
                    {asideData.map ( item =>(
                        <Link key={item.href} to={item.href}> 
                            <motion.div className="flex items-center mt-2 gap-4 p-4 text-sm font-medium rounded-lg hover:bg-[#8CA5CD17] transition-colors mb-2 "> 
